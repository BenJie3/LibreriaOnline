...
let cartItems = window.initialCart ? Object.keys(window.initialCart).map(bookId => {
    ...
    return {
        book: {
            ...
            format: "physical",
            stock: 999,
            image: item.image || (books.find(b => b.id === parseInt(bookId)) || {}).image || "https://via.placeholder.com/50"
        },
        quantity: parseInt(item.quantity) // Asegurar que la cantidad sea un número
    };
}) : [];
        ...
        bookCard.innerHTML = `
            ...
            <div class="book-info">
                ...
                <div class="book-actions">
                    <form method="POST" class="add-to-cart-form" data-book-id="${book.id}">
...
function renderCartItems() {
    ...
    cartItems.forEach((item, index) => {
        const subtotal = item.book.price * item.quantity; // Ahora price es un número
        total += subtotal;
...
function openEbookReader(bookId) {
    ...
    document.getElementById('reader-content').innerHTML = book.content;
    document.getElementById('current-page').textContent = '1';
    document.getElementById('total-pages').textContent = '10';
    
    document.getElementById('ebook-reader').classList.add('active');
    document.getElementById('overlay').classList.add('active');
}

function closeEbookReader() {
    document.getElementById('ebook-reader').classList.remove('active');
